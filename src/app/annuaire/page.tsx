...
import { geocodeCity, haversineKm } from "@/lib/geocode";
...
export default function AnnuairePage() {
  ...
  const [proCoords, setProCoords] = useState<Record<string, { lat: number; lng: number }>>({});
  ...
  useEffect(() => {
    ...
    getProfessionals()
      .then((data) => {
        if (!cancelled) setProfessionals(data.filter((p) => p.status === "approved"));
      })
  ...
  // Geocode all pro cities when userCoords is set
  const geocodeAllPros = useCallback(async (pros: Professional[]) => {
    const results: Record<string, { lat: number; lng: number }> = {};
    await Promise.all(
      pros.map(async (p) => {
        if (!p.id) return;
        if (p.lat !== undefined && p.lng !== undefined) {
          results[p.id] = { lat: p.lat, lng: p.lng };
          return;
        }
        const city = p.locations?.[0]?.city ?? p.city;
        const coords = await geocodeCity(city);
        if (coords) results[p.id] = coords;
      })
    );
    setProCoords(results);
  }, []);

  useEffect(() => {
    if (userCoords && professionals.length > 0) {
      geocodeAllPros(professionals);
    }
  }, [userCoords, professionals, geocodeAllPros]);

  const availableProfessions = useMemo<Profession[]>(() => {
    const seen = new Set<Profession>();
    for (const p of professionals) seen.add(p.profession);
    return Array.from(seen);
  }, [professionals]);

  const filtered = useMemo<(Professional & { distanceKm?: number })[]>(() => {
    const cityNorm = cityQuery.trim().toLowerCase();

    let result = professionals
      .filter((p) => {
        if (selectedProfession !== ALL_KEY && p.profession !== selectedProfession) return false;
        if (cityNorm && !userCoords) {
          const cities = [p.city, ...(p.locations ?? []).map((l) => l.city)].map((c) => c.toLowerCase());
          if (!cities.some((c) => c.includes(cityNorm))) return false;
        }
        return true;
      })
      .map((p) => {
        if (!userCoords || !p.id || !proCoords[p.id]) return { ...p, distanceKm: undefined };
        const km = haversineKm(userCoords.lat, userCoords.lng, proCoords[p.id].lat, proCoords[p.id].lng);
        return { ...p, distanceKm: km };
      });

    if (userCoords) {
      result = result.filter((p) => p.distanceKm === undefined || p.distanceKm <= radiusKm);
      result.sort((a, b) => (a.distanceKm ?? 9999) - (b.distanceKm ?? 9999));
    }

    return result;
  }, [professionals, selectedProfession, cityQuery, userCoords, proCoords, radiusKm]);

  return (
    <main className="min-h-screen bg-axe-black">
      <div className="max-w-6xl mx-auto px-4 pt-6">
        <Link href="/" className="inline-flex items-center gap-1.5 text-sm text-axe-muted hover:text-axe-white transition-colors">
        ...
        <div className="space-y-3">
          ...
          {!loading && (
            <p className="text-sm text-axe-muted pt-1">
              ...
              {filtered.length > 1 ? "professionnels disponibles" : "professionnel disponible"}
            </p>
          )}
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center space-y-4">
            <div className="w-16 h-16 rounded-2xl bg-axe-charcoal border border-white/5 flex items-center justify-center text-3xl">🔍</div>
            <div className="space-y-2">
              <p className="text-axe-white font-semibold text-lg">Aucun professionnel trouvé</p>
              <p className="text-axe-muted text-sm max-w-sm">
                {cityQuery || selectedProfession !== ALL_KEY || userCoords
            ...
            {(cityQuery || selectedProfession !== ALL_KEY || userCoords) && (
              <button
                onClick={() => { setSelectedProfession(ALL_KEY); setCityQuery(""); setUserCoords(null); }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((pro) => (
              <ProDirectoryCard key={pro.id} pro={pro} distanceKm={pro.distanceKm} />
            ))}
          </div>
        )}
      </section>
